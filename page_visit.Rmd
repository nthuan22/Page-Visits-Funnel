...
visit_no_cart <- visits_cart %>% filter(is.na(cart_time)) #%>% nrow(visit_no_cart)
...
checkout_no_purchase <- all_data %>% filter(is.na(purchase_time))
checkout_no_purchase <- nrow(checkout_no_purchase)
...
all_data <- all_data %>% mutate(time_to_purchase = purchase_time - visit_time)
...
time_to_purchase <- all_data %>% summarize(mean_time = mean(time_to_purchase, na.rm = TRUE))
time_to_purchase
